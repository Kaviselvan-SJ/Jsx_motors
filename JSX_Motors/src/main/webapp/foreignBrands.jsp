...
<title>Foreign Car Brands - JSX Motors</title>
...
<body>
	...
	<header>
		<nav>
			...
			<ul class="nav-links">
				...
				<li><a href="foreignBrands.jsp" style="color: var(--accent);">Foreign
						Brands</a></li>
	...
	<section>
		<div class="section-title">
			<h2>Foreign Car Brands</h2>
			<p>Luxury and performance from global manufacturers</p>
		</div>
		<div class="brand-grid">
			<%
			try {
				Statement stmt = con.createStatement();
				ResultSet rs = stmt.executeQuery("SELECT * FROM cars WHERE origin='Foreign'");
				while (rs.next()) {
			...
			<div class="car-card">
				...
				<form action="addToCart" method="post">
					<input type="hidden" name="itemId" value="<%=rs.getInt("id")%>">
					<input type="hidden" name="name" value="<%=rs.getString("model")%>">
					<input type="hidden" name="brand"
						value="<%=rs.getString("brand")%>"> <input type="hidden"
						name="type" value="car"> <input type="hidden" name="price"
